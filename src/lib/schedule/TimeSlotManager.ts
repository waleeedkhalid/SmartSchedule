...
export class TimeSlotManager {
   ...
   */
  isFacultyAvailable(
    faculty: { availableSlots?: TimeSlot[] },
    slot: TimeSlot
  ): boolean {
    // If no availability data, assume available (backward compatibility)
    if (!faculty.availableSlots || faculty.availableSlots.length === 0) {
      return true;
    }

    // Check if the slot overlaps with any of the faculty's available slots
    return faculty.availableSlots.some((availableSlot) =>
