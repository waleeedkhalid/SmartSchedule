...
export function LoadReviewTable({
  initialRows = DEFAULT_ROWS,
  balancedTargetHours = 6,
  title = "Teaching Load Review",
  subtitle = "Inspect instructor loads and imbalances",
}: LoadReviewTableProps): React.ReactElement {
  const [rows] = useState<LoadRow[]>(initialRows); // setter unused (read-only demo)
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      [r.instructor, r.courseCode, r.section].some((v) =>
