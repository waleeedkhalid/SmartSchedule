...
export async function GET(request: NextRequest) {
  try {
    ...
    const instructors = (facultyProfiles || []).map(profile => {
      ...
      const utilizationPercent = maxLoad > 0 
        ? Math.round((currentHours / maxLoad) * 100) 
        : 0
    ...
    const avgUtilization = totalInstructors > 0
      ? Math.round(instructors.reduce((sum, i) => sum + i.utilization_percent, 0) / totalInstructors)
      : 0
