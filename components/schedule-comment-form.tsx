...
export function ScheduleCommentForm({ sections, onCommentCreated }: ScheduleCommentFormProps) {
  const [commentText, setCommentText] = useState("");
  const [commentType, setCommentType] = useState<"general" | "section">("general");
  const [selectedSectionId, setSelectedSectionId] = useState<string>("");
  const [isSubmitting, setIsSubmitting] = useState(false);

  const characterCount = commentText.length;
  const maxCharacters = 2000;
  const isValid = commentText.trim().length > 0 && commentText.length <= maxCharacters;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {sections.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No sections available. You must be assigned to sections to leave section-specific feedback.
                </p>
              ) : (
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              <span className={characterCount === 0 ? "text-muted-foreground" : ""}>
                {characterCount > 0 ? `${characterCount} / ${maxCharacters} characters` : ""}
              </span>
              {characterCount > maxCharacters * 0.9 && (
                <span className={characterCount >= maxCharacters ? "text-destructive font-medium" : "text-amber-600"}>
          ...
          <Button
            type="submit"
            disabled={!isValid || isSubmitting || (commentType === "section" && !selectedSectionId)}
