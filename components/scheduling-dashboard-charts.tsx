...
/**
 * Fetch scheduling dashboard stats from API
 * Separated from component for use with React Query
 */
async function fetchSchedulingDashboardStats(
  termId?: string
...
export function SchedulingDashboardChartsNew({ termId }: Props = {}) {
  ...
  // Use React Query for data fetching with automatic caching and background updates
  const {
    data: dashboardStats,
    isLoading: loading,
    error,
    ...
    queryKey: ["scheduling-dashboard-stats", termId],
    queryFn: () => fetchSchedulingDashboardStats(termId),
    staleTime: 5 * 60 * 1000, // Cache for 5 minutes
    gcTime: 10 * 60 * 1000, // Keep in cache for 10 minutes
    refetchOnWindowFocus: true,
    enabled: isMounted, // Only fetch when component is mounted
  });
  ...
  const courseTypeData = chartData?.courseTypeData || null;
  const instructorLoadData = chartData?.instructorLoadData || null;
  const capacityData = chartData?.capacityData || null;
  const radarData = chartData?.radarData || null;
