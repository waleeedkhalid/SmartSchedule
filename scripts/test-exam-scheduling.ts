...
async function testExamScheduling() {
    ...
    // Create sample exam variables (14 SWE courses) - only course_code, course_level (recommended), and duration_minutes
    const examVariables: ExamVariable[] = [
        { course_code: 'SWE 211', course_level: 2, duration_minutes: 120 },
        { course_code: 'SWE 314', course_level: 3, duration_minutes: 120 },
        { course_code: 'SWE 312', course_level: 3, duration_minutes: 120 },
        { course_code: 'SWE 381', course_level: 3, duration_minutes: 120 },
        { course_code: 'SWE 321', course_level: 3, duration_minutes: 120 },
        { course_code: 'SWE 333', course_level: 3, duration_minutes: 120 },
        { course_code: 'SWE 444', course_level: 4, duration_minutes: 120 },
        { course_code: 'SWE 434', course_level: 4, duration_minutes: 120 },
        { course_code: 'SWE 496', course_level: 4, duration_minutes: 120 },
        { course_code: 'SWE 477', course_level: 4, duration_minutes: 120 },
        { course_code: 'SWE 482', course_level: 4, duration_minutes: 120 },
        { course_code: 'SWE 466', course_level: 4, duration_minutes: 120 },
        { course_code: 'SWE 455', course_level: 4, duration_minutes: 120 },
        { course_code: 'SWE 497', course_level: 4, duration_minutes: 120 },
    ];
    ...
    for (let i = 4; i <= 20; i++) {
        ...
        for (let j = 0; j < numCourses; j++) {
            const randomCourse = examVariables[Math.floor(Math.random() * examVariables.length)];
    ...
    console.log(`   Max possible assignments: ${examDays.length * examTimeSlots.length * examRooms.length} (without TBD)`);
    ...
    const result = await solveExamCSP(examVariables, config, (progress) => {
        if (progress.assigned % 5 === 0 || progress.assigned === examVariables.length) {
            console.log(`   Progress: ${progress.assigned}/${progress.total} assigned (${progress.backtracks} backtracks)`);
        }
    });
    ...
    console.log(`   Assigned: ${result.stats.assigned} (${Math.round(result.stats.assigned / result.stats.total_exams * 100)}%)`);
    ...
    for (const [courseCode, assignment] of result.assignments.entries()) {
        if (count < 5) {
            console.log(`   ${courseCode}: ${assignment.date} at ${assignment.time} in ${assignment.room}`);
            count++;
        }
    }
        ...
        for (const courseCode of courses) {
            ...
            if (assignment) {
                ...
                if (studentExams.has(key)) {
                    console.log(`   ⚠️  ${studentId} has conflict: ${courseCode} and ${studentExams.get(key)} at ${assignment.date} ${assignment.time}`);
                    conflictCount++;
                }
    ...
    // Summary
    if (result.stats.assigned === result.stats.total_exams && conflictCount === 0) {
